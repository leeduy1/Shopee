  ...
  constructor() {
    ...
    this.instance.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        // Lỗi Unauthrized(401) có rất nhiều trường hợp
        // -Token không đúng
        // -Không truyền token
        // -Token hết hạn
        ...
        if (isAxiosUnauthorizedError<ErrorResponse<{ name: string; message: string }>>(error)) {
          ...
          const { url } = config
          // Trường hợp Token hết hạn và request đó ko phải là request refresh token
          ...
          if (isAxiosExpiredTokenError(error) && url !== URL_REFRESH_TOKEN) {
            ...
            return this.refreshTokenRequest.then((access_token) => {
              ...
              // Tiếp tụ gọi lại request cũ vừa bị lỗi
